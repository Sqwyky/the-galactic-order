/**
 * THE GALACTIC ORDER - Terrain Chunk System (Quadtree LOD)
 *
 * The key to seamless orbit-to-surface: a quadtree that subdivides
 * terrain into chunks at different levels of detail.
 *
 * How it works:
 * 1. The planet surface is a flat grid (from the camera's local perspective)
 * 2. Chunks NEAR the camera are small + high detail
 * 3. Chunks FAR from the camera are large + low detail
 * 4. As camera moves, chunks split (subdivide) or merge (combine)
 * 5. Each chunk's heightmap is generated by a WebWorker off-thread
 *
 * This gives us "infinite" detail under the player while keeping
 * the triangle count manageable for 60FPS.
 *
 * Quadtree structure:
 *   Level 0: 1 chunk covering 1024m × 1024m (far away)
 *   Level 1: 4 chunks of 512m × 512m
 *   Level 2: 16 chunks of 256m × 256m
 *   Level 3: 64 chunks of 128m × 128m
 *   Level 4: 256 chunks of 64m × 64m (near camera)
 *   Level 5: 1024 chunks of 32m × 32m (right under player)
 */
...
export const TERRAIN_CONFIG = {
    // Base chunk size in world units
    chunkWorldSize: 64,
...
// NMS-style BOLD, SATURATED colors — inspired by 70s sci-fi book covers.
// NMS uses vivid, almost painterly palettes. Not muted or realistic.
const NMS_COLORS = [
...
export class TerrainChunkMesh {
    ...
    buildMesh(chunkData, config) {
        ...
        // Data is now (res × res) = (33 × 33) — exactly matching PlaneGeometry vertex count.
        ...
        for (let i = 0; i < vertCount; i++) {
            ...
            if (elevation <= config.waterLevel) {
                ...
                // Use height differences to neighbors as a proxy for slope
...
// ============================================================
// TERRAIN MANAGER — orchestrates chunks + worker
// ============================================================
...
export class TerrainManager {
     ...
     * Called every frame — determines which chunks to show/hide/generate.
     */
    ...
    /**
     * Determine which chunks should exist based on camera position.
     * Single LOD level — all chunks are the same size for seamless terrain.
     * This matches NMS's approach for ground-level exploration:
     * consistent detail, no LOD stitching artifacts.
     */
    _getDesiredChunks(camX, camZ) {
        const desired = new Map();
        const chunkSize = this.config.chunkWorldSize;
        const radius = this.config.viewRadius || 8; // chunks in each direction
    ...
    _requestChunk(chunkInfo) {
        const { x, y, depth, worldSize, distance } = chunkInfo;
        const key = `${depth}:${x}:${y}`;
        ...
        // Add to priority queue (closer = higher priority)
        // Use chunk grid coordinates as offset into global heightmap.
        // All chunks share the same planet seed — the chunk position determines
        // WHICH part of the global pattern they sample, ensuring seamless edges.
        this.generationQueue.push({
            ...
            params: {
                ...
                chunkX: x,
                chunkY: y,
                chunkSize: this.config.chunkResolution - 1, // cells, not vertices
                rule: this.planetData.rule,
                seed: this.planetData.seed, // Same seed for all chunks
    ...
    _onWorkerMessage(e) {
        ...
        // We need to match by position since the worker may have
        // multiple jobs in flight
        let matchKey = null;
    ...
    createWater(fogColor, sunDirection, skyTopColor, skyBottomColor) {
        ...
        const mat = new THREE.ShaderMaterial({
            ...
            fragmentShader: /* glsl */ `
                ...
                void main() {
                    ...
                    // Wave crest foam — appears on top of waves
                    float waveHeight = normal.y; // Flatter normal = steeper wave
                    float crestFoam = smoothstep(0.92, 0.98, waveHeight) * foamNoise * 0.6;

                    // Shore/shallow foam — denser near shore
        ...
        this.waterMesh.position.y = -0.1; // At water level
    ...
    updateWater(time, cameraPosition) {
        ...
        if (cameraPosition) {
            ...
            // Water follows camera horizontally
            if (this.waterMesh) {
